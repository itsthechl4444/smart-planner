...
<body>
    <div class="app-bar">
        ...
        <ul id="task-dropdown" class="dropdown-content">
            <li><a href="{{ route('tasks.edit', $task->id) }}"><span class="material-symbols-outlined">edit</span>Edit Task</a></li>
            <li>
...
@endif
    ...
    @if ($task->attachments)
        @php
            $fileType = pathinfo($task->attachments, PATHINFO_EXTENSION);
        @endphp

        @if (in_array($fileType, ['jpg', 'jpeg', 'png', 'gif']))
            <!-- Display image directly with adjusted size -->
            <p><strong>Attachment:</strong></p>
            <img src="{{ asset('storage/' . $task->attachments) }}" alt="Task Attachment" class="task-image img-fluid">
        @else
            <!-- Provide link for non-image files -->
            <p><strong>Attachments:</strong> 
                <a href="{{ asset('storage/' . $task->attachments) }}" target="_blank">Download Attachment</a>
            </p>
        @endif
    @endif
</div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Prevent multiple submissions of the mark as completed form
