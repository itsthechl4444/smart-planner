...
<body>
    ...
    <div class="app-bar">
        ...
        <h1 class="app-bar-title">Edit Expense</h1>
        ...
        <div class="container">
            <form id="edit-expense-form" action="{{ route('expenses.update', $expense->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    <input type="text" class="form-control" id="name" name="name" placeholder="Expense Name" value="{{ $expense->name }}" required>
                    @error('name')
                ...
                <!-- Amount -->
                <div class="form-group">
                    <input type="number" class="form-control" id="amount" name="amount" placeholder="Amount" value="{{ $expense->amount }}" required>
                    @error('amount')
                        <span class="text-danger">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control" id="payment_method" name="payment_method" required>
                        @foreach ($paymentMethods as $paymentMethod)
                            <option value="{{ $paymentMethod }}" {{ $expense->payment_method == $paymentMethod ? 'selected' : '' }}>{{ $paymentMethod }}</option>
                        @endforeach
                    </select>
                ...
                <!-- Notes -->
                <div class="form-group">
                    <textarea class="form-control" id="notes" name="notes" placeholder="Notes">{{ $expense->notes }}</textarea>
                    @error('notes')
                        <span class="text-danger">{{ $message }}</span>
                    @enderror
                </div>
                <!-- Attachment with Current Display and Preview -->
                <div class="form-group">
                    ...
                    @endif
                    <input type="file" class="form-control-file" id="attachment" name="attachment" accept="image/*,application/pdf">
                    @error('attachment')
                        <span class="text-danger">{{ $message }}</span>
                    @enderror
